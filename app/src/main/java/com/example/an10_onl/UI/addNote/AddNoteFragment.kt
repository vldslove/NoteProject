...
class AddNoteFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.backToNoteList.setOnClickListener {
    ...
    private fun validateTitle(): String? {
        titleInputLayout?.editText?.let {
            val result = com.example.an10_onl.utils.validation.validateTitle(it.text.toString())
            return when (result) {
                is Invalid -> {
    ...
    private fun validateMessage(): String? {
        messageInputLayout?.editText?.let {
            val result = com.example.an10_onl.utils.validation.validateMessage(it.text.toString())
            return when (result) {
                is Invalid -> {
